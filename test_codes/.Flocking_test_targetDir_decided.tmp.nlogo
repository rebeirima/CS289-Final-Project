...
turtles-own [
  ...
  d_target           ;; target direction
  c_target           ;; certainty of target direction
...
to find-flockmates  ;; turtle procedure
  ;; Get rumors from flockmates? And average their rumors?
...
to color-direction
  let c_new (hsb d_target 100 (c_target * 100) );; Saturation picked 100 constant
  set color c_new
end
...
;      set rumors replace-item (speaker_idx mod 5) rumors (list nearest-neigh-dir nearest-neigh-cert)
      ...
      insert-rumor (nearest-neigh-dir + 10) nearest-neigh-cert
...
;      ask nearest-neighbor [set rumors replace-item (speaker_idx mod 5) rumors (list my-dir my-cert)]
      ask nearest-neighbor [insert-rumor (my-dir + 10) my-cert]
...
;Calculates new target certainty from the current rumors and d_target values
; the goal was to generate a function that would (a) allow certainty to increase if the rumor directions agreed
; (a2) the increase being proportional to their certainties, (b) decrease certainty if there is high variation,
; even if each rumor was of high certainty, (c) low-certainty rumors should have less effect than high-certainty ones.
; This had to be done in a way that certainty would stay between (0,1) always.
; The solution: two components
; 1 - a certainty-weighted average of scaled variances, where variance (0-180 deg) between rumor and target
;     is a value from (1-0) in respective mapping, i.e: (0-180) -> (1-0).
;     This is smaller for high varience, and so encourages certainty to go down if rumors are conflicting.
; 2 - a varience-weighted average of the goal certainties, where each certainty value ci is adjusted to
